...
public class TabLayout extends HorizontalScrollView {
    ...
    /**
     * This tabs display all tabs concurrently and are best used with content that benefits from
     * quick pivots between tabs. It will be OK when tabs is less or more than one screen, the
     * tabs will display equal width ,it also is SCROLLABLE. It added up Fixed and Scrollable.
     *
     * It is Changed by guohuai and zhiyi.
     *
     * @see #setTabModeBySelf(int)
     */
    public static final int MODE_SCROLLABLE_EVEN_SHORT = 10;
     ...
     */
    @RestrictTo(GROUP_ID)
    @IntDef(value = {MODE_SCROLLABLE_EVEN_SHORT})
    @Retention(RetentionPolicy.SOURCE)
     ...
     * Set the behavior mode for the Tabs in this layout. The valid input options are:
     *
     * The TabMode is MODE_SCROLLABLE, but it will be ok no mater the tab is a larger number of tabs
     * or a shorter number of tabs.
     *
     * </ul>
     */
    public void setTabModeBySelf(@SelfMode int selfMode) {
    ...
    private void updateTabViewLayoutParams(LinearLayout.LayoutParams lp) {
        if (mMode == MODE_FIXED && mTabGravity == GRAVITY_FILL) {
            ...
            // 自定义布局显示（统一设置为MODE_SCROLLABLE，tab数量少也可以平均宽度）
            if (mSelfMode == MODE_SCROLLABLE_EVEN_SHORT){
                lp.weight = 1;
            }else {
                lp.weight = 0;
            }
